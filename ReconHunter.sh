...
echo "Running Amass..."
amass enum -passive -d $Domain > 1-1-Amass.txt 2>/dev/null
...
cat 1-5-Total-Valid-Scraping.txt 2-3-BruteForce-Domains.txt | tr A-Z a-z | sort | uniq > 3-1-Combine-SubDomains.txt
cat 3-1-Combine-SubDomains.txt | sed 's/^[*.]*//g' > 3-2-Clean-Domains.txt

> 3-3-Have-Wildcard.txt
cat 3-2-Clean-Domains.txt | while read line; do
host -t A *.$line | grep "has address" | cut -d " " -f1 | sed 's/^*.//g' >> 3-3-Have-Wildcard.txt
done

cat 3-2-Clean-Domains.txt 3-3-Have-Wildcard.txt | sort | uniq -u > 3-4-Not-Have-Wildcard.txt

> 3-5-Root-Wildcard.txt
cat 3-3-Have-Wildcard.txt | while read line; do
tmp=""
while(true)
do
...
cat 1-3-Total-Scraping.txt 2-3-BruteForce-Domains.txt 3-6-3-Spider-SubDomains.txt | tr A-Z a-z | sort | uniq > 4-1-Combine-For-Takeover.txt

cp ../Resources/providers.json providers.json
../Tools/bin/SubOver -l 4-1-Combine-For-Takeover.txt -v > 4-2-Test-Takeover.txt 2>&1
rm providers.json

echo "Running The Second Check..."
cat 4-1-Combine-For-Takeover.txt | while read sub;do
host $sub | grep alias | cut -d " " -f1,6 > take.tmp
cat take.tmp >> 4-2-Test-Takeover.txt
...
censys search --pages 1000 "$Domain" > 5-1-Censys-Result.txt
...
> 9-1-WayBackURLs.txt
cat 3-6-4-Final-SubDomains.txt | while read line; do
echo $line | ../Tools/bin/waybackurls >> 9-1-WayBackURLs.txt
done

# Extract Parameters & Files
cat 9-1-WayBackURLs.txt | sort -u | ../Tools/bin/unfurl keys | sort | uniq > 9-2-Parameters.txt
cat 9-1-WayBackURLs.txt | sort -u | grep -P "\w+\.js(\?|$)" | sort | uniq > 9-3-JS-Files.txt
cat 9-1-WayBackURLs.txt | sort -u | grep -P "\w+\.php(\?|$)" | sort | uniq > 9-4-PHP-Files.txt
cat 9-1-WayBackURLs.txt | sort -u | grep -P "\w+\.jsp(\?|$)" | sort | uniq > 9-5-JSP-Files.txt
